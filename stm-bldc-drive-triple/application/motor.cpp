...
void Motor::calibrate() {
	...
	while (1) {
		...
		int sensor = adc_dma_results[dma_index];
...
void Motor::assign_angle(uint32_t power, int32_t angle) {
	...
	*pwm[0] = 3199 + (int32_t)(sintab[(angle            ) & (N-1)]) * power / SINMAX;
	*pwm[1] = 3199 + (int32_t)(sintab[(angle + N23      ) & (N-1)]) * power / SINMAX;
	*pwm[2] = 3199 + (int32_t)(sintab[(angle + N23 + N23) & (N-1)]) * power / SINMAX;
